...
*/
function exercise1() {
    let orderRut = clients.sort((a, b) => a.rut.localeCompare(b.rut))
    return orderRut.map((orderRut) => orderRut.id);
}
/**
Ejercicio 2
...
function exercise2() {
    const arrClientes = [];
    clients.forEach(c => {
        const resultReduce = accounts.filter(f => f.clientId == c.id).reduce((acc, cur) => acc + cur.balance, 0);
        arrClientes.push({ name: c.name, sumaSaldo: resultReduce });
        arrClientes.sort((a, b) => b.sumaSaldo - a.sumaSaldo);
    });
    return arrClientes;
}
...
function exercise3() {
    let resultB = {};
    var rutC = [];
    let orderBanks = banks.sort((a, b) => a.name.localeCompare(b.name))
    orderBanks.forEach(element => {
        rutC = accounts.filter(acc => acc.bankId == element.id);
        const clientsRut = [];
        rutC.forEach(aux => {
            clients.forEach(clients => {
                if (clients.id == aux.clientId) {
                    clientsRut.push(clients.rut);
                }
            });
        })
        const bankN = element.name;
        resultB[bankN] = [... new Set(clientsRut)]
    });
    return resultB;
}
/**
...
function exercise4() {
    ...
    accounts.forEach(element => {
        if (element.bankId == 1) {
            if (element.balance > 25000) {
                arrScot.push(element.balance)
            }
        }
    });
    arrScot.sort(function (a, b) { return b - a });
    return arrScot;
}
/**
Ejercicio 5
...
function exercise5() {
    const arrBancos = [];
    banks.forEach(c => {
        const resultReduce = accounts.filter(f => f.bankId == c.id).reduce((acc, cur) => acc + cur.balance, 0);
        arrBancos.push({ name: c.name, sumaSaldo: resultReduce });
        arrBancos.sort((a, b) => b.sumaSaldo - a.sumaSaldo);
    });
    return arrBancos;
}
/**
...
function exercise6() {
    ...
    banks.forEach(element => {
        ...
        const clientList = [];
        accounts.forEach(f => {
            if (f.bankId == element.id) {
                clientIdList.push(f.clientId);
                clientList.push(f);
            }
        });
        const list = getDuplicates(clientIdList);
        const duplicateList = [...new Set(list)]
        const resultList = [];

        clientList.forEach(f => {
            if (!duplicateList.includes(f.clientId)) {
                resultList.push(f);
            }
        })
        result.push({ name: element.name, cantidadClientes: resultList.length });
    });
    return result;
}
/**
...
function exercise7() {
    ...
    banks.forEach(element => {
        ...
        const clientList = accounts.filter(f => f.bankId == element.id).sort((a, b) => a.balance - b.balance);
        result.push({
            name: element.name, idCliente: clientList[0].clientId, balance: clientList[0].balance
        });
